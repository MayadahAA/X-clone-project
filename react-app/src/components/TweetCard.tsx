...
export default function TweetCard() {
  ...
  const delPost = (id: string) => {
    axios.delete(`https://64d8b3c25f9bf5b879ce7999.mockapi.io/posts/${id}`)
    .then(() => {
        setGetPost(
            getPost.filter((del) => {
                return del.id !== id;
  ...
  return (
    <>
      {getPost.map((post) => (
        <div
          className="flex flex-col mx-1 text-white h-auto  border-b-2 border-slate-700 p-3"
          key={post.id}
        >
          <div className="flex flex-col p-2">
            <div className="flex justify-between ">
              ...
              <div className="">
                <button onClick={() => setdelBtn(!delBtn)}>
                  <svg
                    ...
                    <path
                     ...
                     <button className="bg-black" onClick={() => delPost(post.id)}>{delBtn ? '' : 'delete'}</button> 
                </button>
              </div>
            </div>
